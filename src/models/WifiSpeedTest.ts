...
export interface IWifiSpeedTest extends Document {
  ...
  // Test Method
  testMethod: 'ookla' | 'fast.com' | 'manual' | 'speedtest-net' | 'cloudflare' | 'ndt7' | 'credentials_only';
...
const WifiSpeedTestSchema = new Schema<IWifiSpeedTest>({
  ...
  testMethod: {
    type: String,
    enum: ['ookla', 'fast.com', 'manual', 'speedtest-net', 'cloudflare', 'ndt7', 'credentials_only'],
    required: true
  },
